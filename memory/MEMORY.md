...
- [Word SVG metadata survival](word-svg-metadata.md) — Word strips `<metadata>` but preserves the root `id` attribute; encode all recoverable data in `id` as `lxs2-{base64url(JSON)}`
